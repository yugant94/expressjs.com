...
_Middleware_ Funktionen sind Funktionen, die Zugriff auf das [Anfrageobjekt](/{{ page.lang }}/4x/api. tml#req) (`req`), das [Antwort-Objekt](/{{ page.lang }}/4x/api.html#res) (`res`) und die nächste Middleware-Funktion im Request-Antwort-Zyklus der Anwendung. Die nächste Middleware-Funktion wird üblicherweise durch eine Variable namens `next` bezeichnet.
...
Binde Middleware auf Anwendungsebene an eine Instanz des [App-Objekts](/{{ page.lang }}/4x/api.html#app), indem du die `app.use()` und `app. ETHOD()` Funktionen, wobei `METHOD` die HTTP-Methode der Anfrage ist, die die Middleware-Funktion in Kleinbuchstaben behandelt (wie GET, PUT, oder POST)
...
{% include admonitions/note.html content="`next('route')` wird nur in Middleware-Funktionen funktionieren, die mit den `app.METHOD()` oder `router.METHOD()` Funktionen geladen wurden." %}
...
<h2 id='middleware.error-handling'>Fehler beim Umgang mit Middleware</h2>

<div class="doc-box doc-notice" markdown="1">
Bei der Fehlerbehebung der Middleware werden immer _vier_ Argumente verwendet. Sie müssen vier Argumente angeben, um es als Middleware-Funktion zu identifizieren. Selbst wenn du nicht das `next` Objekt verwenden musst, musst du es angeben, um die Signatur zu pflegen. Andernfalls wird das `next` Objekt als reguläre Middleware interpretiert und wird Fehler nicht bearbeiten.
</div>
...
Details über die Middleware finden Sie hier: [Fehlerbehandlung](/{{ page.lang }}/guide/error-handling.html).
...
- [express.static](/en/4x/api.html#express.static) dient statischen Assets wie HTML-Dateien, Bilder und so weiter.
- [express.json](/en/4x/api.html#express.json) analysiert eingehende Anfragen mit JSON Payloads. **HINWEIS: Verfügbar mit Express 4.16.0+**
- [express.urlencoded](/en/4x/api.html#express.urlencoded) analysiert eingehende Anfragen mit URL-kodierten Payloads.  **HINWEIS: Verfügbar mit Express 4.16.0+**

<h2 id='middleware.third-party'>Drittanbieter-Middleware</h2>
...
Für eine partielle Liste der Middleware-Funktionen von Drittanbietern, die häufig mit Express verwendet werden, siehe: [Middleware](../resources/middleware.html).
